...
// 从wif获取私钥公钥地址等相关信息
eNeo.getInfoFromWIF = function (wif) {
...
// 只转Nep5资产快捷方法
eNeo.transferNep5 = function (wif, assertId, toAddr, amount) {
...
// 只转全局资产快捷方法
eNeo.transferGlobalCoin = function (wif, utxos, assertId, toAddr, amount) {
  if (assertId === "neo") {
    assertId = eNeo.config.neoId;
  }
  if (assertId === "gas") {
    assertId = eNeo.config.neoGasId;
  }
...
// 直接调用合约（与utxo无关）
eNeo.callC = function (wif, scHash, method, params) {
  let prikey = ThinNeo.Helper.GetPrivateKeyFromWIF(wif);
  let pubkey = ThinNeo.Helper.GetPublicKeyFromPrivateKey(prikey);
  let address = ThinNeo.Helper.GetAddressFromPublicKey(pubkey);
...
// 调用合约底层方法
eNeo.callC2 = function (wif, globalCoinParams, callParams) {
  let prikey = ThinNeo.Helper.GetPrivateKeyFromWIF(wif);
  let pubkey = ThinNeo.Helper.GetPublicKeyFromPrivateKey(prikey);
  let address = ThinNeo.Helper.GetAddressFromPublicKey(pubkey);
  let addressHash = ThinNeo.Helper.GetPublicKeyScriptHash_FromAddress(address);
...
// rpc调用
eNeo.getRpc = function (method, ...args) {
  const urlStr = eNeo.config.apiUrl + "?jsonrpc=2.0&id=1&method=" + method + "&params=" + JSON.stringify(args);
  return axios.get(urlStr)
};

// rpc调用
eNeo.postRpc = function (method, ...args) {
  let params = {
    "jsonrpc": "2.0",
    "method": method,
    "id": "1",
  ...
  return axios.post(eNeo.config.apiUrl, params)
};

// 处理参数，返回sb-ScriptBuilder
eNeo.emitParams = function (scHash, params) {
  let sb = new ThinNeo.ScriptBuilder();
